# Build This App — Store-Ready Package for Your Best App

> **Level app project** (not a toy snippet). Complete this after the level theory/exercises.

**Time:** 8–12 hours  
**Why it matters:** Shipping is a skill; employers ask if you have released.

---

## Product

You are building: **Store-Ready Package for Your Best App**

## Acceptance checklist

- [ ] App icon + splash
- [ ] Package name / bundle id finalized
- [ ] Android signing notes (local keystore NOT in git)
- [ ] iOS metadata checklist
- [ ] Play/App Store listing draft copy
- [ ] Privacy policy URL placeholder

## Steps

1. Choose chat, cart, or habit app
2. Follow Level 15 theory for signing
3. Build appbundle / ipa as far as accounts allow
4. Write STORE_LISTING.md

## Definition of done

STORE_LISTING.md + screenshots folder complete

## After this

Level 16 architecture refactor

## Link to mega tutorials

See also [`Full-App-Tutorials/README.md`](../../Full-App-Tutorials/README.md) for larger employer-grade apps (chat, ride-hailing, delivery, marketplace, …).

---

## ShopEase note

If this level’s `Capstone/` continues the **ShopEase** multi-level shop, you may do **either**:

1. This standalone **Build-This-App**, or  
2. The ShopEase Capstone for the level  

**Recommendation:** Standalone app first (cleaner portfolio repo), ShopEase if you want one long continuous project.
